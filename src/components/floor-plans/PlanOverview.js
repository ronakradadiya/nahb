...
import { formatPrice, formatSqft, formatPricePerSqft } from "@/lib/formatters";
...
export function PlanOverview({ plan, className, ...props }) {
  ...
  return (
    <div className={cn("", className)} {...props}>
      ...
      <div className="mb-6">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            ...
            {ogName && <p className="text-ebony/50 mt-1">{ogName}</p>}
          </div>
      ...
      {/* Description */}
      {description && (
        <p className="text-ebony/70 leading-relaxed mb-8">{description}</p>
      )}
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8">
        ...
        <SpecItem
          ...
          value={formatPricePerSqft(basePrice, sqft)}
        />
      </div>
        ...
        <div>
          ...
          <ul className="space-y-2">
            {highlights.map((highlight, index) => (
              <li key={index} className="flex items-start gap-2 text-ebony/70">
                <svg
